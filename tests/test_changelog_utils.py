...
def test_get_commit_changes_breaking_change_from_diff(mock_repo):
    ...
    breaking_change_keywords = ["breaking", "breaking change", "deprecated", "removed"]
    result = get_commit_changes(repo, commit1, commit2, breaking_change_keywords)
...
def test_get_commit_changes_with_parsed_commits(mock_repo):
    ...
    )  # Assuming no breaking changes in these mocks
