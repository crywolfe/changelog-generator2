...
def test_get_commit_changes_modified(temp_git_repo, capfd):
    ...
    try:
        ...
        assert (
            "initial.txt" in changes["modified_files"]
        ), f"Expected 'initial.txt' in modified files, got {changes['modified_files']}"
        assert (
            len(changes["commit_messages"]) == 1
        ), f"Expected 1 commit message, got {len(changes['commit_messages'])}"
        assert (
            changes["commit_messages"][0] == "Modified initial file"
        ), f"Unexpected commit message: {changes['commit_messages'][0]}"
