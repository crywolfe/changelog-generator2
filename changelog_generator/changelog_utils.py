...
def parse_commit_message(message: str) -> Dict[str, Optional[str]]:
    ...
    match = re.match(
        r"^(?P<type>\w+)(?P<breaking>!)?(?:\((?P<scope>[^)]*)\))?: (?P<description>.*)",
        message,
    )
    if match:
...
def get_commit_changes(
    repo: git.Repo,
    commit1: Union[str, git.Commit],
    commit2: Union[str, git.Commit],
    breaking_change_keywords: List[str],
) -> Dict[str, List[str]]:
    """
    ...
    for diff_detail in changes.get("diff_details", []):
        ...
        if any(change in patch for change in structural_changes):
            ...
            logger.info(
                f"Structural change detected in {diff_detail.get('file', 'unknown file')}"
            )
